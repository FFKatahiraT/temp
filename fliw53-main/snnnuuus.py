...
gamewin = pygame.image.load('gamewin.jpg')
...
while not crashed:
    ...
    color=gameDisplay.get_at((int(x+32.5),int (y+92)))
    ...
    if y<10:
        y=10
    if y>display_height*0.8 and life_counter>0:
        life_counter-=1
        x = (0)
        y = (200)
    if life_counter<=0 and y>display_height*0.75:
        y=display_height*0.75
        gameDisplay.blit(gameover,(0,0))
    if display_width*0.9 < x:
        gameDisplay.blit(gamewin,(0,0))
        x=display_width*0.91
    gameDisplay.blit(chelImg2, (x,y))
